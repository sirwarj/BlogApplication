...
router.get('/', function(req, res, next) {
  ...
  blogs.find({},{},function(err,blog){
    categories.find({},{},function(err,category){
      res.render('index', { posts: blog, categories: category, moment: moment });
    });
  });
});
...
router.get('/post/add', function(req, res, next) {
  var categories = db.get('categories');
  categories.find({},{},function(err,category){
    res.render('addpost', { categories: category });
  });
});
...
router.post('/post/add',[
  ...
  if(!result.isEmpty()){
    categories.find({},{},function(err,category){
      res.render('addpost', { categories: category, errors:errors });
    });
  }else{
    // Insert Post
    posts.insert({
...
router.get('/show/:id', function(req, res, next){
  ...
  blogs.find(req.params.id, {}, function(err, blog){
    categories.find({}, {}, function(err, category){
      res.render('show', { posts: blog, categories: category, moment: moment });
    });
  });
});
...
router.get('/posts/show/', function(req, res, next){
  ...
  if(name){
    blogs.find({category:name}, {}, function(err, blog){
      categories.find({}, {}, function(err, category){
        res.render('show_search', { posts: blog, categories: category, moment: moment, search:name });
      });
    });
  }
  if(author){
    blogs.find({author:author}, {}, function(err, blog){
      categories.find({}, {}, function(err, category){
        res.render('show_search', { posts: blog, categories: category, moment: moment, search:author });
      });
    });
  }
  if(title){
    blogs.find({title:title}, {}, function(err, blog){
      categories.find({}, {}, function(err, category){
        res.render('show_search', { posts: blog, categories: category, moment: moment, search:title });
      });
    });
  }
});
